...
template<>
bool smaller<int>(const int& a, const int& b)
{
	//The <int> in smaller<int> may be omitted (type deduction).
	return abs(a) < abs(b); //Comparison according to the absolute amount!
}
...
int main()
{
	...
	//In the following two statements, the compiler controller
	//by the data type vector<int> of the parameter iV, generates
	//from the above template the functions biggestValue(const std::vector<int>&) and print(const std::vector<int>&),
	//as well the implicitly called function is smaller(const int&, const int&).
	std::cout << "The biggest absolute value of ";
	print(iV);
	std::cout << " is " << abs(biggestValue(iV)) << '\n';
	std::vector<double> dV{ 1.09,2.2,79.6,-1.9,2.7,100.9,18.8,99.9 };
	//Generate the overloaded functions biggestValue(const std::vector<double>&) and
	//print(const std::vector<double>&) (and the called function
	//smaller(const double&, const double&)):
